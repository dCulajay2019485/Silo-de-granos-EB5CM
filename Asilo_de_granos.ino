...
#define servo 3                   // Directivas de preprocesador
...
float capacidad;           // Declara una variable de tipo float llamada "capacidad"
...
pantalla_grupo.print("Desea servir?   ");          // Imprime el texto Desea servir? en el LCD  
    ...
    pantalla_grupo.setCursor(0,0);                 // Establece la posición del cursor en la columna 0 y la fila 0 del LCD
    pantalla_grupo.print(" Cierre la Tapa ");      // Imprime el texto Cierre la Tapa en el LCD
    pantalla_grupo.setCursor(0,1);                 // Establece la posición del cursor en la columna 0 y la fila 1 del LCD
    pantalla_grupo.print("                ");      // Imprime espacios en blanco del LCD
   }
...
switch(tecla){                                  // que evaluará el valor de tecla
   ...
   pantalla_grupo.print("   1 onza   ");        // Se imprime "1 onza" en el LCD  
      ...
      Servo_grupo.write(90);                    // Se mueve el servo a una posición de 90 grados
    ...
    for(int i = 0; i < 3; i++){                  // Se verifica antes de cada iteración para determinar si el bucle debe continuar ejecutándose
     pantalla_grupo.setCursor(0,1);              // Se configura el cursor en la posición (0, 1) del LCD
     pantalla_grupo.print("      producto     ");// Se imprime "producto" en el LCD
     delay(500);                                 // Espera de 500 milisegundos 
     pantalla_grupo.setCursor(0,1);              // Se configura el cursor en la posición (0, 1) del LCD
     pantalla_grupo.print("  escaso   ");        // Se imprime "escaso" en el LCD
     delay(500);                                 // Espera de 500 milisegundos
    }
    break;                                       // Después de ejecutar las acciones 
  }
  ...
  case '2':                                      // Presionar tecla 2
   ...
   pantalla_grupo.setCursor(0,1);                // Se configura el cursor en la posición (0, 1) del LCD
   pantalla_grupo.print("   2 onzas   ");        // Se imprime "2 onzas" en el LCD
      Serial.println(capacidad);                 // Escribe la cantidad que desea
      Servo_grupo.write(90);                     // Se mueve el servo a una posición de 90 grados
  ...
  case '3':                                      // Presionar tecla 3
   ...
   pantalla_grupo.setCursor(0,1);                // Se configura el cursor en la posición (0, 1) del LCD
   pantalla_grupo.print("   3 onzas    ");       // Se imprime "3 onzas" en el LCD
      Serial.println(capacidad);                 // Escribe la cantidad que desea
      Servo_grupo.write(90);                     // Se mueve el servo a una posición de 90 grados
  ...
  case '4':                                      // Presionar tecla 4
   ...
   pantalla_grupo.setCursor(0,1);                // Se configura el cursor en la posición (0, 1) del LCD
   pantalla_grupo.print("  4 onzas   ");         // Se imprime "4 onzas" en el LCD
      Serial.println(capacidad);                 // Escribe la cantidad que desea
      Servo_grupo.write(90);                     // Se mueve el servo a una posición de 90 grados
  ...
  case '5':                                      // Presionar tecla 5
   ...
   pantalla_grupo.setCursor(0,1);                // Se configura el cursor en la posición (0, 1) del LCD 
   pantalla_grupo.print("  5 onzas   ");         // Se imprime "5 onzas" en el LCD
      Serial.println(capacidad);                 // Escribe la cantidad que desea
      Servo_grupo.write(90);                     // Se mueve el servo a una posición de 90 grados
  ...
  case '6':                                      // Presionar tecla 6
   ...
   pantalla_grupo.setCursor(0,1);                // Se configura el cursor en la posición (0, 1) del LCD 
   pantalla_grupo.print("6 onzas ");             // Se imprime "6 onzas" en el LCD
      Serial.println(capacidad);                 // Escribe la cantidad que desea
      Servo_grupo.write(90);                     // Se mueve el servo a una posición de 90 grados
